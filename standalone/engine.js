...
function buildReport(birth){
  ...
  career.linkedFields=synthesize(education.streams,career.fields);
...
const SYN_DOMAINS=[
 ["MEDICINE",["medicine","medical","surgeon","surgery","dentist","dental","nursing","healthcare","health","pharma","pharmacy","physician","clinical"]],
 ["PSYCHOLOGY",["psychology","counsel"]],
 ["ENGINEERING",["engineering","mechanical","civil","structural","manufacturing","metallurgy","aeronautic","aviation","electronics","construction","geology","mining"]],
 ["IT",["information technology","software","computer","data science","data","computing","telecom","electronics & communication"]],
 ["LAW",["law","judiciary","legal"]],
 ["ARTS",["fine art","art","design","fashion","music","performing","interior","graphic","beauty","luxury","photograph","film","cinematograph","entertainment","architecture"]],
 ["MEDIA",["media","journalism","writing","publishing","communication","advertising","public relations","language"]],
 ["DEFENCE",["defence","defense","police","army","military","law enforcement"]],
 ["SCIENCE",["science","research","mathematics","physics","forensic","investigation","statistics"]],
 ["HOSPITALITY",["hospitality","hotel","tourism","food","beverage","dairy","marine","shipping"]],
 ["AGRI",["agriculture","farming","agri"]],
 ["SPIRITUAL",["spiritual","religion","vedic","occult","healing","philosophy"]],
 ["FINANCE",["finance","banking","account","wealth","stock","economics","commerce","trading","business","management"]],
 ["TEACHING",["teaching","academia","education","professor"]],
 ["ADMIN",["government","administration","civil service","political","politics","diplomacy","leadership","social"]],
 ["ENERGY",["power","energy","oil"]],["REALESTATE",["real estate"]]];
function synClassify(title){const t=(title||"").toLowerCase();
  for(const [d,kws] of SYN_DOMAINS)if(kws.some(k=>t.includes(k)))return d;return "GENERAL";}
const SYN_COMBOS={
 "ARTS|MEDICINE":["Plastic & cosmetic surgeon","Reconstructive / maxillofacial surgeon","Medical illustrator","Art / music therapist"],
 "ARTS|ENGINEERING":["Industrial / product designer","Architectural & structural designer","Automotive / aerospace designer"],
 "ARTS|IT":["UX / UI designer","Game / technical artist","Creative technologist","Motion-graphics / VFX artist"],
 "ARTS|FINANCE":["Luxury-brand manager","Art dealer / gallery business","Creative director (advertising)"],
 "ARTS|MEDIA":["Film director / cinematographer","Creative & content director","Animation producer"],
 "ARTS|ADMIN":["Cultural-affairs / museum administrator","Arts-policy & council officer"],
 "ARTS|HOSPITALITY":["Hospitality interior designer","Culinary artist / pastry chef","Event & experience designer"],
 "ARTS|TEACHING":["Fine-arts / design educator","Music / drama teacher"],
 "ARTS|LAW":["Entertainment & IP lawyer"],
 "ENGINEERING|MEDICINE":["Biomedical engineer","Medical-devices / prosthetics engineer","Clinical / rehabilitation engineer"],
 "ENGINEERING|IT":["Robotics / embedded-systems engineer","Hardware-software systems engineer","IoT solutions engineer"],
 "ENGINEERING|FINANCE":["Engineering project / cost manager","Quantitative analyst (fintech)","Techno-commercial manager"],
 "ENGINEERING|DEFENCE":["Defence / aerospace engineer","Military systems & weapons engineer","Naval / submarine engineer"],
 "ENGINEERING|ADMIN":["Engineering-services (govt) officer","Infrastructure & public-works administrator"],
 "ENGINEERING|SCIENCE":["R&D / materials engineer","Aerospace research engineer"],
 "ENGINEERING|TEACHING":["Engineering professor / technical trainer"],
 "IT|MEDICINE":["Health-informatics specialist","Medical-software / telemedicine engineer","Bioinformatics / genomics analyst"],
 "IT|FINANCE":["Fintech engineer / product manager","Algorithmic-trading developer","Data scientist (banking)"],
 "IT|LAW":["Cyber-law & data-privacy specialist","Legal-tech engineer"],
 "IT|DEFENCE":["Cyber-security / cyber-defence specialist","Defence software engineer"],
 "IT|MEDIA":["Digital-media engineer","AdTech / streaming-platform developer"],
 "IT|ADMIN":["e-Governance / digital-public-services architect","Government IT officer"],
 "IT|SCIENCE":["Computational scientist","AI / machine-learning researcher"],
 "IT|HOSPITALITY":["Travel-tech / hospitality-platform developer"],
 "FINANCE|MEDICINE":["Hospital administrator","Health-economics / pharma-finance analyst","Medical-insurance & actuarial specialist"],
 "FINANCE|IT":["Fintech product / business manager","ERP & business-systems consultant"],
 "FINANCE|LAW":["Corporate / tax lawyer","Company secretary","Compliance & risk officer"],
 "FINANCE|ADMIN":["Revenue / treasury-services officer","Public-finance & budget administrator"],
 ...
 "LAW|FINANCE":["Corporate / banking lawyer","Tax & GST consultant","Mergers & acquisitions counsel"],
 "LAW|ADMIN":["Judge / magistrate","Civil-services (judicial/administrative) officer","Public-policy & legislative advisor"],
 "LAW|MEDIA":["Media & defamation lawyer","Legal journalist"],
 "LAW|DEFENCE":["Military / JAG legal officer"],"LAW|IT":["Cyber-law / IP attorney"],
 "LAW|MEDICINE":["Medico-legal consultant / medical-negligence lawyer"],
 "SCIENCE|MEDICINE":["Medical / clinical researcher","Pharmacologist","Geneticist / molecular biologist"],
 ...
 "MEDIA|ADMIN":["Public-relations / information-services officer","Government communications / spokesperson"],
 "MEDIA|DEFENCE":["Defence / war correspondent"],
 "PSYCHOLOGY|MEDICINE":["Psychiatrist","Clinical psychologist","Neuropsychologist"],
 ...
 "ADMIN|MEDICINE":["Public-health administrator","Health-services / hospital director"],
 ...
 "SPIRITUAL|MEDICINE":["Integrative / alternative-medicine practitioner (Ayurveda, naturopathy)"],
 "SPIRITUAL|TEACHING":["Yoga / Vedic-studies teacher"],
 "SPIRITUAL|PSYCHOLOGY":["Spiritual counsellor / wellness coach"]};
function synCore(title){let t=title;for(const sep of ["/",",","&","("])t=t.split(sep)[0];return t.trim();}
function synFallback(e,c,ed,cd){const cc=synCore(c),ee=synCore(e);
  if(ed===cd||ed==="GENERAL"||cd==="GENERAL")return `${cc} specialist (${ee} background)`;
  return `${cc} with a ${ee} specialisation`;}
function synthesize(eduStreams,careerFields,topE,topC,limit){topE=topE||2;topC=topC||2;limit=limit||6;
  const es=eduStreams.slice(0,topE),cs=careerFields.slice(0,topC);if(!es.length||!cs.length)return [];
  const pairs=[];cs.forEach((c,ci)=>es.forEach((e,ei)=>pairs.push([ei+ci,e,c])));pairs.sort((a,b)=>a[0]-b[0]);
  const out=[],seen={};
  for(const [,e,c] of pairs){const et=e.title,ct=c.title,ed=synClassify(et),cd=synClassify(ct);
    let roles=SYN_COMBOS[ed+"|"+cd]||SYN_COMBOS[cd+"|"+ed];if(!roles)roles=[synFallback(et,ct,ed,cd)];
    for(const r of roles.slice(0,2)){if(seen[r])continue;seen[r]=1;
      out.push({title:r,educationField:et,careerField:ct,basis:`Education in ${et} combined with a career in ${ct}.`});
