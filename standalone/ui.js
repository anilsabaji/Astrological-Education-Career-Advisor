...
function render(rep){
  ...
  $("pdfPanel").innerHTML=renderDownload(rep);
}
...
function renderDownload(rep){
  return `<div class="card"><h2>&#11015; Download Report</h2>
    <div class="pdf-cta">
      <p>Save the complete, <strong>vibrantly coloured</strong> report &mdash; every section
      (Overview &amp; Dasha, Education, Career, FAQ, Best Periods, Shadbala and Charts) &mdash;
      to your device in either format.</p>
      <div style="display:flex;gap:12px;justify-content:center;flex-wrap:wrap;margin:6px 0 4px;">
        <button class="pdf-btn" onclick="downloadHTML()">&#11015; Download as HTML</button>
        <button class="pdf-btn" onclick="printReport()">&#128424; Download as PDF</button>
      </div>
      <p style="font-size:.82rem;margin-top:14px;"><strong>HTML</strong> downloads a self-contained file you can
      reopen or print any time. For <strong>PDF</strong>, choose <strong>"Save as PDF"</strong> in the print
      dialog and keep <strong>"Background graphics"</strong> enabled &mdash; the report is laid out centred on A4.</p>
...
// Build a self-contained HTML file of the current report and download it.
window.downloadHTML=function(){
  const styles=[...document.querySelectorAll("style")].map(s=>s.outerHTML).join("\n");
  const area=document.getElementById("reportArea").cloneNode(true);
  area.querySelectorAll(".tabs,#pdfPanel,.no-print").forEach(el=>el.remove());
  area.querySelectorAll(".tab-panel").forEach(p=>{p.classList.add("active");p.style.display="block";});
  const nameRaw=(document.getElementById("name").value||"report");
  const name=nameRaw.replace(/[^a-z0-9]+/gi,"_").replace(/^_+|_+$/g,"")||"report";
  const doc=`<!DOCTYPE html><html lang="en"><head><meta charset="utf-8">`+
    `<meta name="viewport" content="width=device-width, initial-scale=1.0">`+
    `<title>Astro Adviser report - ${esc(nameRaw)}</title>${styles}</head><body>`+
    `<header class="site-header"><div class="wrap"><span class="brand"><span class="logo">&#9789;</span> Astro Adviser</span>`+
    `<span class="tagline">Education &amp; Career &middot; KP + Parashara</span></div></header>`+
    `<main class="wrap">${area.innerHTML}</main>`+
    `<footer class="site-footer"><div class="wrap">`+
    `<p class="dev-credit">Developed by Dr. Anil Sabaji, email: <a href="mailto:anilsabaji@gmail.com">anilsabaji@gmail.com</a></p>`+
    `<p class="foot-note">Generated by the Astro Adviser (KP + Parashara). For guidance and self-reflection only.</p>`+
    `</div></footer></body></html>`;
  const blob=new Blob([doc],{type:"text/html;charset=utf-8"});
  const a=document.createElement("a");
  a.href=URL.createObjectURL(blob);a.download=`astro_report_${name}.html`;
